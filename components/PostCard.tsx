...
export const PostCard: React.FC<PostCardProps> = ({ 
  ...
  // 核心逻辑：获取所有需要展示的图片
  // 优先使用 post.images 数组；如果没有，则兼容旧版 mediaUrl
  ...
  const getGridClass = (count: number) => {
      if (count === 1) return ""; 
      if (count === 2 || count === 4) return "grid-cols-2 max-w-[240px]"; 
  ...
  return (
    <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden shadow-xl mb-16 max-w-3xl mx-auto transition-all hover:shadow-2xl hover:bg-white/15 relative group animate-fade-in-up">
        ...
        <div className="flex-1">
          {isEditing ? (
            ...
            <>
              ...
              <div className="inline-flex items-center gap-1.5 mt-3 text-sm text-amber-200 font-mono bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/30 px-3 py-1 rounded-full shadow-sm backdrop-blur-sm">
      ...
      <div className="w-full bg-black/20 p-4 min-h-[50px]">
        {post.mediaType === 'video' && post.mediaUrl ? (
             ...
             displayImages.length === 1 ? (
                 // 1张图：原样显示（大图）
                 <div className="flex justify-center">
                    <SafeImage src={displayImages[0]} alt={post.title} className="max-w-full max-h-[500px] object-contain w-auto h-auto rounded-lg shadow-lg border border-white/10" />
                 </div>
             ) : (
                 // 多张图：朋友圈九宫格布局
                 <div className={`grid gap-1.5 mx-auto ${getGridClass(displayImages.length)}`}>
                     {displayImages.map((img, idx) => (
                         <SafeImage 
                            ...
                            alt={`${post.title}-${idx}`} 
                            // 正方形裁剪，类似朋友圈
                            className="w-full aspect-square object-cover rounded hover:opacity-90 transition-opacity cursor-pointer bg-gray-800 border border-white/10 shadow-sm"
                         />
                     ))}
                 </div>
             )
