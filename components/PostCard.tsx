...
const SafeImage: React.FC<SafeImageProps> = ({ src, alt, className, onClick }) => {
  ...
  if (status === 'error' && !tempSrc) {
    ...
    return (
      <div className={`w-full bg-gray-900 border-2 border-red-500 rounded-lg p-2 flex flex-col items-center text-center gap-2 ${className}`}>
...
export const PostCard: React.FC<PostCardProps> = ({ 
  ...
  const getGridClass = (count: number) => {
      if (count === 1) return ""; 
      if (count === 2 || count === 4) return "grid-cols-2 max-w-sm"; 
        ...
        {post.mediaType === 'video' && post.mediaUrl ? (
             ...
             displayImages.length === 1 ? (
                 ...
                 <div className="flex justify-center min-h-[100px]">
                    <SafeImage src={displayImages[0]} alt={post.title} className="max-w-full max-h-[80vh] object-contain w-auto h-auto rounded-lg" />
                 </div>
             ) : (
                 // 多张图：朋友圈九宫格
